...
YEARS = [2023, 2024, 2025]  # 2026 준비되면 추가
...
def load_team_list():
    ...
    try:
        ...
        # C열(2번째 index), P열(15번째 index)
        team_col = cols[2]   # 팀명
        pw_col   = cols[15]  # 비밀번호
        result = df[[team_col, pw_col]].dropna(subset=[team_col])
        result = result.rename(columns={team_col: "팀명", pw_col: "비밀번호"})
        result = result[result["팀명"].astype(str).str.strip() != ""]
    ...
    with tab3:
        ...
        with col_b:
            ...
            <button onclick="window.print()"
              style="width:100%;padding:8px;background:#3B6D11;color:#EAF3DE;
